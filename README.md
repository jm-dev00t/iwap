...
- 전체 페이지 h1 폰트를 Cormorant Garamond에서 Inter Bold로 통일해 시인성을 개선했습니다.
- 명령 센터 제목 크기를 워크플로 페이지와 맞췄습니다.
- 사이드바 IW 로고 클릭 시 명령 센터(홈)로 이동하도록 Link 처리했습니다.
- 워크플로·승인대기·감사로그·보고서 현황 타일 4개를 모든 페이지 공통 고정 상단바(`TopBar` 컴포넌트)로 분리했습니다. 사이드바와 상단바를 동시에 사용하는 레이아웃으로 전환했습니다.
- 상단바 우측에 백엔드 연결 상태(연결됨/연결 중/미연결)를 실시간으로 표시합니다. 각 페이지의 개별 연결 상태 표시는 제거했습니다.
- 상단바 타일을 레이블 좌측·숫자 우측 컴팩트 레이아웃으로 변경하고, 마우스 오버 시 숫자와 아이콘에 primary 색상 호버 효과를 추가했습니다.
- 승인 대기 타일의 앰버 강조 색상을 제거했습니다.
- 보고서 페이지 좌측 카드 목록에 시나리오별 이전/다음 페이지 버튼을 추가했습니다.
- 명령 센터 채팅 영역 높이를 조정했습니다 (312px / 데스크탑 396px).

### 승인 플로우 안정화

- 승인/반려 API 타임아웃을 기본 1.5초에서 15초로 늘렸습니다.
- 승인 mutation 버그 수정: `onSuccess`에서 UI 상태를 업데이트하도록 변경했고, API 실패 시 에러 알림을 표시합니다.

### Gmail SMTP 실제 발송 연동

- `docker-compose.yml` 백엔드 서비스에 SMTP/Slack 환경변수를 추가했습니다.
- `DeliveryService`의 `@Value` 바인딩 오류(`iwap.integrations.mode` → `IWAP_INTEGRATION_MODE`)를 수정했습니다.
...
| Agent 실제 동작 + 상태 기계 | ✅ 완료 | 2026-05-15 |
| OpenAI 플래너 멀티턴 | ✅ 완료 | 2026-05-15 |
| Gmail SMTP 실제 발송 연동 | ✅ 완료 | 2026-05-15 |
| 승인 플로우 안정화 (타임아웃·뮤테이션 버그) | ✅ 완료 | 2026-05-15 |
| 보고서 이메일 본문 포함 | ✅ 완료 | 2026-05-15 |
| UI 폰트·레이아웃 통일 | ✅ 완료 | 2026-05-15 |
| 공통 고정 상단바 (TopBar) 분리 | ✅ 완료 | 2026-05-15 |
| 상단바 백엔드 연결 상태 표시 통합 | ✅ 완료 | 2026-05-15 |
| 보고서 카드 시나리오별 페이지 버튼 | ✅ 완료 | 2026-05-15 |
| 채팅 영역 높이 조정 | ✅ 완료 | 2026-05-15 |
