...
UI 통일, 실제 외부 발송 연동, 승인 플로우 안정화, 보고서 페이지 개선 작업을 진행했습니다.

**UI/UX**
- 전체 페이지 h1 폰트를 Cormorant Garamond에서 Inter Bold로 통일해 시인성을 개선했습니다.
- 명령 센터 제목 크기를 워크플로 페이지와 맞췄습니다.
- 사이드바 IW 로고를 클릭하면 명령 센터(홈)로 이동하도록 Link 처리했습니다.
- 워크플로·승인대기·감사로그·보고서 현황 타일 4개를 모든 페이지 공통 고정 상단바(`TopBar`)로 분리했습니다. 사이드바와 상단바를 동시에 사용하는 레이아웃으로 변경했습니다.
- 승인 대기 타일의 앰버 강조 색상을 제거했습니다.
- 보고서 페이지 좌측 카드 목록에 시나리오별 이전/다음 페이지 버튼을 추가했습니다.

**승인 플로우 안정화**
- 승인/반려 API 타임아웃을 1.5초에서 15초로 늘렸습니다.
...
- `NotifierAgent`가 보고서 산출물 내용을 이메일 본문에 포함하도록 `buildEmailBody` 메서드를 추가했습니다.
...
| UI 폰트 통일 및 레이아웃 개선 | ✅ 완료 | 2026-05-15 |
